 ...
 * CS 225 fall 2011
 * Final Project
 * Class Name: setAlarmHigh
 * Class Description: This class is for setting the High alarm parameters by using the JSlider.
 * When the setAlarmHigh button is pressed, a new frame will be generated and user can slide the 
...
public class setAlarmHigh extends JFrame
{
	JLabel label;
	JButton button;
	private static int heartRate_Data_h = 100; 
	private static int Respiratory_Data_h = 80; 
	private static int SpO2D_Data_h = 110;
	private static int BPSystolic_Data_h = 140; 
	private static int BPDiastolic_Data_h = 90;
	private static int Temperature_Data_h = 100;
	
	public setAlarmHigh()
	{
		JPanel p1 = new JPanel(new GridLayout(12,1));
		//setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));
		JSlider heartRateh = new JSlider(JSlider.HORIZONTAL,40,120,heartRate_Data_h);
		...
		JSlider BPSystolich = new JSlider(JSlider.HORIZONTAL,90,180,BPSystolic_Data_h);
		JSlider BPDiastolich = new JSlider(JSlider.HORIZONTAL,50,140,BPDiastolic_Data_h);
		JSlider Temperatureh = new JSlider(JSlider.HORIZONTAL,100,120,Temperature_Data_h);
		
		//to initialize and modify the 6 sliders
		heartRateh.setMajorTickSpacing(5);         
	...
	public static void setHeartRate_Data_h(int heartRate_Data_h) {
		setAlarmHigh.heartRate_Data_h = heartRate_Data_h;
	}
